...
function ensureActiveQuestId(stateLike) {
  const activeQuestStillVisible =
    stateLike.activeQuestId && isQuestAvailableForSelection(stateLike, stateLike.activeQuestId)

  if (activeQuestStillVisible) {
    return stateLike
  }

  const nextActiveQuestId =
    QUEST_CHAIN_ORDER.find((questId) => isQuestAvailableForSelection(stateLike, questId)) || null
...
function buildInitialQuestState() {
  const unlockedQuestIds = uniqueIds(DEFAULT_UNLOCKED_QUEST_IDS)

  return ensureActiveQuestId({
    unlockedQuestIds,
    activeQuestId: getFirstActiveQuestId(unlockedQuestIds),
    completedQuestIds: [],
    claimedQuestIds: [],
    rewardClaimedQuestIds: [],
    objectiveProgress: createInitialObjectiveProgress(unlockedQuestIds),
  })
}
...
function applyObjectiveEvent(stateLike, objectiveType, itemId, quantity = 1) {
  ...
  return ensureActiveQuestId(unlockAvailableQuests(completeEligibleQuests(progressedState)))
}
...
export const useQuestStore = create((set, get) => ({
  ...
  claimQuestRewards: (questId) => {
    ...
    rewards
      ...
      .forEach((r) => useCharacterStore.getState().unlockCharacter(r.value))
