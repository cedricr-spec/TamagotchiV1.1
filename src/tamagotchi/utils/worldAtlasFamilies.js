...
function chooseMetadataVariantForCell(
  ...
  const ordered = [...definitions].sort((a, b) => {
    const scoreA = hashUnit(...seedParts, a.id);
    const scoreB = hashUnit(...seedParts, b.id);
    if (scoreA !== scoreB) return scoreA - scoreB;
    return a.id.localeCompare(b.id);
  });

  return (
    ordered.find((definition) => !nearbyVariantIds.has(definition.id)) ||
    ordered[0]
  );
}
...
function createDecorEligibilityCache(cells, chunkX, chunkY) {
  ...
  for (let gridX = ownedBounds.startX; gridX <= ownedBounds.endX; gridX += 1) {
    for (let gridY = ownedBounds.startY; gridY <= ownedBounds.endY; gridY += 1) {
      ...
      if (cell.terrainType === TERRAIN_TYPES.GRASS_LIGHT && isCellBaseFullTerrain(cell)) {
        ...
        if (hasNearbyTerrainType(cells, gridX, gridY, TERRAIN_TYPES.SAND, 2)) {
          pools.near_sand_grass_light_full.push(cell);
        }
        if (hasNearbyTerrainType(cells, gridX, gridY, TERRAIN_TYPES.ROAD, 2)) {
          pools.near_road_grass_light_full.push(cell);
        }
      } else if (
        cell.terrainType === TERRAIN_TYPES.GRASS_DARK &&
...
function getDeterministicMetadataPoolOrder(pool, familyId, chunkX, chunkY, definition = null) {
  // Hoist all per-family / per-chunk constants out of the per-cell loop.
  // getMetadataPoolScore previously recomputed these for every cell; with pre-hoisting
  // they are computed once and captured in the closure, saving ~4–6 fn calls per cell.
  const seedId = getMetadataPlacementSeedId(familyId, definition);
  const noiseScale = getMetadataPlacementNoiseScale(familyId);
  const clusterWeight = getMetadataPlacementClusterWeight(familyId);
  const familyPhase = hashUnit("metadata-pool-phase", seedId, familyId);
  const driftNoiseScale = Math.max(0.05, noiseScale * 0.5);
  const pocketSeed = `metadata-pocket:${seedId}`;
  const driftSeed = `metadata-drift:${seedId}`;
  const definitionId = definition?.id || "default";
  // Pre-add the chunk-level offsets so each cell only adds its own gridX/Y.
  ...
  const scored = pool.map((cell) => {
    const pocket =
      valueNoise(
        cell.gridX + pocketBaseX,
        cell.gridY + pocketBaseY,
        pocketSeed,
        noiseScale
      ) * clusterWeight;
    const drift =
      valueNoise(
        cell.gridX * 0.82 + driftBaseX,
        cell.gridY * 0.74 + driftBaseY,
        driftSeed,
        driftNoiseScale
      ) * (1 - clusterWeight * 0.38);
    ...
    return {
      cell,
      score: pocket * 0.36 + drift * 0.18 + laneBreak * 0.24 + stagger * 0.14 + micro * 0.08 + centerBias,
      tie: hashUnit("metadata-pool-tie", familyId, chunkX, chunkY, cell.gridX, cell.gridY),
    };
  });
  ...
  scored.sort((a, b) => {
    ...
    if (a.cell.gridY !== b.cell.gridY) return a.cell.gridY - b.cell.gridY;
    return a.cell.gridX - b.cell.gridX;
  });

  return scored.map(({ cell }) => cell);
}
...
function buildChunkLayout(chunkX, chunkY, atlasData) {
  ...
  const eligibility = createDecorEligibilityCache(cells, chunkX, chunkY);
...
function buildCombinedWorldLayout(playerX, playerY, atlasData, viewport = DEFAULT_VIEWPORT) {
  ...
  forEachChunkInRange(chunkWindow.activeChunkRange, (chunkX, chunkY) => {
    const chunk = getChunkLayout(atlasData, chunkX, chunkY);
...
const _treeCollisionSourceLogged = import.meta.env.DEV ? new Set() : null;
...
export function getWorldAtlasCollisionBounds(item) {
  ...
  if (
    ...
    if (import.meta.env.DEV && item.entry?.group === "trees") {
